<template>
  <div class="flex-1 flex flex-col min-h-0">
    <!-- Header -->
    <div class="px-4 md:px-8 pt-4 md:pt-6 pb-3 border-b border-slate-100 flex justify-between items-center shrink-0">
      ...
      <div class="relative">
        ...
        <transition name="dropdown">
          <div v-if="showHistory" class="absolute right-0 top-full mt-2 w-80 bg-white rounded-2xl shadow-xl border border-slate-100 z-30 flex flex-col max-h-[70vh]">
            ...
            <div class="flex-1 overflow-y-auto p-3 space-y-1.5">
              <button @click="startNewChat" class="w-full p-3 text-left bg-indigo-50 hover:bg-indigo-100 rounded-xl text-sm text-indigo-600 font-medium transition">✏️ 新建对话</button>
              ...
              <div v-for="conv in conversations" :key="conv.id" @click="loadConversation(conv)"
                ...
                <div class="text-[10px] text-slate-400 mt-1 handwritten">{{ conv.date }}</div>
              </div>
            </div>
          </div>
        </transition>
    ...
    <!-- Input area -->
    <div class="px-4 md:px-8 py-3 border-t border-slate-100 bg-white shrink-0">
