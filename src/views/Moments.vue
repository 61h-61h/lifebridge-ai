<template>
  <div class="space-y-6">
    ...
    <div v-if="showForm" class="bg-white p-5 rounded-3xl border border-slate-100 shadow-sm space-y-3">
      ...
      <div class="flex gap-1"><button v-for="m in quickMoods" :key="m" @click="form.mood = form.mood === m ? '' : m" class="text-lg px-2 py-1 rounded-lg transition min-w-[44px] min-h-[44px] flex items-center justify-center" :class="form.mood === m ? 'bg-slate-50' : 'hover:bg-slate-50 text-slate-400'">{{ m }}</button></div>
      ...
      <div class="flex gap-2"><button @click="showForm = false" class="flex-1 px-4 py-2.5 bg-slate-100 text-slate-600 text-sm rounded-xl hover:bg-slate-200 transition btn-cancel font-body">取消</button><button @click="addMoment" class="flex-1 px-5 py-2.5 bg-slate-800 text-white text-sm rounded-xl hover:bg-slate-700 hover:scale-[1.02] transition btn-publish font-body">发布</button></div>
    </div>
    <div class="flex gap-2">
      <button @click="filter='all'" :class="filter==='all'?'bg-slate-50 text-slate-600':'bg-white text-slate-500 border border-slate-100'" class="px-4 py-2 rounded-full text-sm transition hover:scale-[1.02] min-h-[36px]">全部</button>
      <button @click="filter='public'" :class="filter==='public'?'bg-slate-50 text-slate-600':'bg-white text-slate-500 border border-slate-100'" class="px-4 py-2 rounded-full text-sm transition hover:scale-[1.02] min-h-[36px]">公共</button>
      <button @click="filter='private'" :class="filter==='private'?'bg-slate-50 text-slate-600':'bg-white text-slate-500 border border-slate-100'" class="px-4 py-2 rounded-full text-sm transition hover:scale-[1.02] min-h-[36px]">🔀 隐私</button>
    </div>
    <div class="space-y-4">
      <div v-for="m in filteredMoments" :key="m.id" class="bg-white p-5 rounded-3xl border border-slate-100 shadow-sm">
        <div class="flex justify-between items-start">
          <div class="flex items-center gap-2"><div class="w-9 h-9 bg-gradient-to-br from-slate-400 to-slate-600 rounded-full flex items-center justify-center text-white text-sm font-body">我</div><span class="text-sm text-slate-400 handwritten">{{ m.createdAt }}</span><span v-if="m.mood" class="text-lg">{{ m.mood }}</span><span v-if="m.isPrivate" class="text-xs">🔀</span></div>
