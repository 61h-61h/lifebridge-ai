<template>
  <div class="p-4 md:p-8 w-full max-w-4xl mx-auto space-y-6">
    ...
    <div v-if="showForm" class="bg-white p-5 rounded-3xl border border-warmer-200 shadow-card space-y-3">
      ...
      <textarea v-model="form.description" rows="3" placeholder="???????..." class="w-full p-3 bg-warmer-100 rounded-2xl text-sm text-body outline-none focus:ring-2 focus:ring-sage-400/30 resize-none placeholder:text-subtle"></textarea>
      <select v-model="form.category" class="w-full p-3 bg-warmer-100 rounded-2xl text-sm outline-none text-body"><option value="career">?? ??</option><option value="study">?? ??</option><option value="life">?? ??</option><option value="relationship">?? ??</option><option value="health">?? ??</option></select>
      ...
      <div class="flex gap-2"><button @click="showForm = false" class="flex-1 px-4 py-2 bg-warmer-200 text-body text-sm rounded-2xl hover:bg-warmer-300 transition btn-cancel font-body">??</button><button @click="addMilestone" class="flex-1 px-6 py-2 bg-navy-500 text-white text-sm rounded-2xl hover:bg-navy-600 hover:scale-105 transition btn-save font-body">????</button></div>
    </div>
    ...
    <div class="relative pl-8 border-l-2 border-warmer-300 space-y-6">
      <div v-for="m in milestonesList" :key="m.id" class="relative">
        ...
        <div class="bg-white p-5 rounded-3xl border border-warmer-200 shadow-card hover:shadow-soft transition">
          <div class="flex justify-between items-start">
            <div>
              <span class="text-xs font-semibold text-sage-400 handwritten">{{ categoryIcon(m.category) }} {{ formatDate(m.date) }}</span>
              <h3 class="font-bold text-navy-500 mt-1 font-heading">{{ m.title }}</h3>
            </div>
            <button @click="deleteMilestone(m.id)" class="text-coral-400 hover:text-coral-500 text-xs btn-delete">??</button>
          </div>
      ...
      <div v-if="milestonesList.length === 0" class="text-center text-muted text-sm py-10">?????????????????? ?</div>
    </div>
  </div>
</template>
<script setup>
...
const categoryIcon = (c) => ({career:'??',study:'??',life:'??',relationship:'??',health:'??'})[c]||'??';
const formatDate = (d) => { const dt=new Date(d); return dt.getFullYear()+'?'+(dt.getMonth()+1)+'?'+dt.getDate()+'?'; };
const getDaysMessage = (d) => { const n=new Date(); n.setHours(0,0,0,0); const t=new Date(d); t.setHours(0,0,0,0); const diff=Math.floor((t-n)/(1000*60*60*24)); if(diff>0) return '???? '+diff+' ?'; if(diff<0){ const a=Math.abs(diff); let m='??? '+a+' ?'; if(a>=365) m+=' ('+Math.floor(a/365)+' ?)'; return m; } return '???????'; };
const checkMilestones = () => { const n=new Date(); n.setHours(0,0,0,0); const reminders=[]; const ms=storage.get(KEYS.MILESTONES); ms.forEach(m=>{ const t=new Date(m.date); t.setHours(0,0,0,0); const diff=Math.floor((n-t)/(1000*60*60*24)); if(diff>=0){ const k=m.id+'-'+diff; if(!notifiedMilestones.value.has(k)){ if(diff===0){reminders.push({id:m.id,title:m.title,message:'???????'});notifiedMilestones.value.add(k)}else if(diff===100){reminders.push({id:m.id,title:m.title,message:'???100????'});notifiedMilestones.value.add(k)}else if(diff===1000){reminders.push({id:m.id,title:m.title,message:'???1000????'});notifiedMilestones.value.add(k)}else if(diff===365){reminders.push({id:m.id,title:m.title,message:'??????????'});notifiedMilestones.value.add(k)}else if(diff>365&&diff%365===0){reminders.push({id:m.id,title:m.title,message:'???'+Math.floor(diff/365)+'????'});notifiedMilestones.value.add(k)} } } }); activeReminders.value=reminders; };
const addMilestone = () => { if(!form.value.title.trim()||!form.value.date) return alert('????????'); storage.add(KEYS.MILESTONES,{...form.value}); form.value={title:'',date:'',description:'',category:'life',image:''}; showForm.value=false; refreshMilestones(); checkMilestones(); };
const deleteMilestone = (id) => { if(confirm('?????')){ storage.delete(KEYS.MILESTONES,id); refreshMilestones(); checkMilestones(); } };
