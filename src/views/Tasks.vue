<template>
  <div class="space-y-6">
    <div class="border-b border-slate-100 pb-4 flex justify-between items-center">
      ...
      <button @click="showForm = !showForm" class="px-5 py-2.5 bg-primary-600 text-white text-sm rounded-xl hover:bg-primary-700 hover:scale-[1.02] transition font-body min-h-[44px]" :class="showForm ? 'btn-cancel' : 'btn-add'">{{ showForm ? '取消' : '+ 新增任务' }}</button>
    </div>
    <div v-if="showForm" class="bg-white p-5 rounded-3xl border border-slate-100 shadow-sm space-y-3">
      <input v-model="form.title" placeholder="任务名称" class="w-full p-3 bg-slate-50 rounded-2xl text-sm outline-none focus:ring-2 focus:ring-primary-200 placeholder:text-slate-300" />
      ...
      <div class="flex items-center gap-3 flex-wrap">
        <label class="flex items-center gap-2 text-sm text-slate-600 cursor-pointer min-h-[44px]"><input type="checkbox" v-model="form.useAI" class="rounded accent-primary-600" /><span>💻 AI 智能优化</span></label>
        <button @click="addTask" :disabled="taskLoading" class="px-6 py-2.5 bg-primary-600 text-white text-sm rounded-xl hover:bg-primary-700 hover:scale-[1.02] transition disabled:opacity-50 btn-save font-body">{{ taskLoading ? 'AI 优化中...' : '确认添加' }}</button>
      </div>
    </div>
      ...
      <div v-for="q in quadrants" :key="q.key" class="p-5 rounded-3xl border-2 min-h-[180px]" :class="q.bg + ' ' + q.border">
        ...
        <div class="space-y-2">
          <div v-for="t in getTasks(q.key)" :key="t.id" class="p-3 bg-white rounded-2xl shadow-sm flex items-start gap-2 border border-slate-100" :class="{'opacity-50 line-through':t.done}">
            <input type="checkbox" :checked="t.done" @change="toggleTask(t)" class="mt-0.5 shrink-0 accent-primary-600" />
            <div class="flex-1 min-w-0"><span class="text-slate-700 font-medium text-sm">{{ t.title }}</span><div v-if="t.aiOptimized" class="text-accent-600 mt-1 text-xs">✨ {{ t.aiOptimized }}</div><div v-if="t.optimizing" class="text-primary-400 mt-1 animate-pulse text-xs">AI 优化中...</div></div>
            <div class="flex gap-1.5 shrink-0"><button @click="optimizeTask(t)" :disabled="t.optimizing" class="text-primary-400 hover:text-primary-600 text-xs min-w-[32px] min-h-[32px]">{{ t.optimizing ? '...' : 'AI' }}</button><button @click="deleteTask(t.id)" class="text-rose-400 hover:text-rose-600 text-xs btn-delete min-w-[32px] min-h-[32px]">✕</button></div>
          </div>
...
const quadrants = [
  ...
  { key:'q3',label:'🟝 紧急不重要',border:'border-primary-200',bg:'bg-primary-50/50',text:'text-primary-600' },
