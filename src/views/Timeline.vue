<template>
  <div class="space-y-6">
    ...
    <div v-if="showForm" class="bg-white p-5 rounded-3xl border border-slate-100 shadow-sm space-y-3">
      ...
      <textarea v-model="form.description" rows="3" placeholder="描述这个里程碑..." class="w-full p-3 bg-slate-50 rounded-2xl text-sm outline-none focus:ring-2 focus:ring-indigo-100 resize-none placeholder:text-slate-300"></textarea>
      ...
      <div v-for="m in milestonesList" :key="m.id" class="relative">
        ...
        <div class="bg-white p-5 rounded-3xl border border-slate-100 shadow-sm hover:shadow-md transition">
          <div class="flex justify-between items-start">
            <div><span class="text-xs font-semibold text-indigo-600 handwritten">{{ categoryIcon(m.category) }} {{ formatDate(m.date) }}</span><h3 class="font-bold text-slate-800 mt-1">{{ m.title }}</h3></div>
