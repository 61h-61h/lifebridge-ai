<template>
  <div class="flex flex-col md:flex-row h-screen bg-slate-50 overflow-hidden">
    ...
    <!-- 手机端顶部栏 -->
    <header class="md:hidden flex items-center justify-between bg-slate-900 text-white px-4 py-3 shrink-0 z-30">
    ...
    <!-- 手机端侧边栏遮罩 -->
    <div v-if="mobileMenuOpen" class="md:hidden fixed inset-0 bg-black/50 z-20" @click="mobileMenuOpen = false"></div>

    <!-- 侧边栏 -->
    <aside 
      :class="mobileMenuOpen ? 'translate-x-0' : '-translate-x-full'"
      class="w-60 bg-slate-900 text-white flex flex-col shrink-0 fixed md:static inset-y-0 left-0 z-20 transition-transform duration-300 md:translate-x-0">
