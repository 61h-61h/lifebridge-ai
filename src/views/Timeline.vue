<template>
  <div class="space-y-6">
    <div class="border-b border-slate-100 pb-4 flex justify-between items-center">
      <div><h1 class="text-2xl font-bold text-slate-800 font-heading">🔔 时光纪念馆</h1><p class="text-sm text-slate-400 mt-1">记录人生中每一个闪光的里程碑</p></div>
      <button @click="showForm = !showForm" class="px-5 py-2.5 bg-primary-600 text-white text-sm rounded-xl hover:bg-primary-700 hover:scale-[1.02] transition font-body min-h-[44px]" :class="showForm ? 'btn-cancel' : 'btn-add'">{{ showForm ? '取消' : '+ 新增里程碑' }}</button>
    </div>
    <div v-if="showForm" class="bg-white p-5 rounded-3xl border border-slate-100 shadow-sm space-y-3">
      <input v-model="form.title" placeholder="里程碑标题" class="w-full p-3 bg-slate-50 rounded-2xl text-sm outline-none focus:ring-2 focus:ring-primary-200 placeholder:text-slate-300" />
      <input v-model="form.date" type="date" class="w-full p-3 bg-slate-50 rounded-2xl text-sm outline-none text-slate-700" />
      <textarea v-model="form.description" rows="3" placeholder="描述这个里程碑..." class="w-full p-3 bg-slate-50 rounded-2xl text-sm outline-none focus:ring-2 focus:ring-primary-200 resize-none placeholder:text-slate-300"></textarea>
      <select v-model="form.category" class="w-full p-3 bg-slate-50 rounded-2xl text-sm outline-none text-slate-700"><option value="career">💼 职业</option><option value="study">📎 学业</option><option value="life">🏠 生活</option><option value="relationship">❤️ 情感</option><option value="health">🩭 健康</option></select>
      ...
      <div class="flex gap-2"><button @click="showForm = false" class="flex-1 px-4 py-2.5 bg-slate-100 text-slate-600 text-sm rounded-xl hover:bg-slate-200 transition btn-cancel font-body">取消</button><button @click="addMilestone" class="flex-1 px-6 py-2.5 bg-primary-600 text-white text-sm rounded-xl hover:bg-primary-700 hover:scale-[1.02] transition btn-save font-body">确认添加</button></div>
    </div>
    <div class="space-y-2" v-if="activeReminders.length > 0">
      <div v-for="reminder in activeReminders" :key="reminder.id" class="p-4 bg-amber-50 border border-amber-200 rounded-2xl flex items-center gap-3"><span class="text-2xl">🎉</span><div><div class="font-bold text-amber-700 text-sm">{{ reminder.title }}</div><div class="text-xs text-amber-600">{{ reminder.message }}</div></div></div>
    </div>
    <div class="relative pl-8 border-l-2 border-primary-100 space-y-6">
      <div v-for="m in milestonesList" :key="m.id" class="relative"><div class="absolute -left-[41px] top-1 w-4 h-4 rounded-full border-2 border-primary-400 bg-white"></div><div class="bg-white p-5 rounded-3xl border border-slate-100 shadow-sm hover:shadow-md transition"><div class="flex justify-between items-start"><div><span class="text-xs font-semibold text-primary-600 handwritten">{{ categoryIcon(m.category) }} {{ formatDate(m.date) }}</span><h3 class="font-bold text-slate-800 mt-1 font-heading">{{ m.title }}</h3></div><button @click="deleteMilestone(m.id)" class="text-rose-400 hover:text-rose-600 text-sm btn-delete min-w-[44px] min-h-[44px] flex items-center justify-center">删除</button></div><p class="text-xs text-slate-400 mt-1">{{ getDaysMessage(m.date) }}</p><p v-if="m.description" class="text-sm text-slate-600 mt-2">{{ m.description }}</p><img v-if="m.image" :src="m.image" class="mt-3 rounded-2xl max-h-60 w-full object-cover" /></div></div>
      <div v-if="milestonesList.length === 0" class="text-center text-slate-400 text-sm py-12">还没有里程碑，记录你的第一个闪光时刻 ✨</div>
    </div>
  </div>
</template>
<script setup>
