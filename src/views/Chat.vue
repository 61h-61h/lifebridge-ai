<template>
  <div class="flex flex-col h-full">
    <div class="border-b border-slate-100 pb-3 flex justify-between items-center">
      ...
      <button @click="showHistory = !showHistory" class="px-3 py-2 bg-slate-100 hover:bg-slate-200 rounded-xl text-sm transition flex items-center gap-1.5 text-slate-600"><span>💬</span><span>历史记录</span><span v-if="conversations.length > 0" class="bg-indigo-500 text-white text-[10px] px-1.5 py-0.5 rounded-full">{{ conversations.length }}</span></button>
    </div>
    <div class="p-2 md:p-3 flex gap-1.5 md:gap-2 flex-wrap">
      <button v-for="prov in providers" :key="prov.key" @click="selectProvider(prov.key)" :class="selectedProvider === prov.key ? 'bg-indigo-600 text-white' : 'bg-white text-slate-500 hover:bg-slate-50 border border-slate-100'" class="px-2.5 md:px-4 py-1.5 md:py-2 rounded-lg md:rounded-xl text-xs md:text-sm transition">{{ prov.icon }} {{ prov.name }}</button>
    </div>
    <div class="flex-1 overflow-y-auto p-3 md:p-4" ref="chatContainer">
    ...
    <transition name="slide">
      <aside v-if="showHistory" class="fixed inset-0 bg-white z-20 flex flex-col md:absolute md:inset-auto md:right-0 md:top-0 md:bottom-0 md:w-72 md:border-l md:border-slate-100 md:shadow-lg">
        <div class="p-4 border-b border-slate-100 flex justify-between items-center"><h2 class="font-bold text-slate-800">💬 对话历史</h2><button @click="showHistory = false" class="text-slate-400 hover:text-slate-600 text-xl">&times;</button></div>
        <div class="flex-1 overflow-y-auto p-3 space-y-2">
          <button @click="startNewChat" class="w-full p-3 text-left bg-indigo-50 hover:bg-indigo-100 rounded-2xl text-sm text-indigo-600 font-medium transition">✨ 新建对话</button>
          <div v-if="conversations.length === 0" class="text-center text-slate-400 text-xs py-8">暂无历史对话</div>
          <div v-for="conv in conversations" :key="conv.id" @click="loadConversation(conv)" :class="currentConvId === conv.id ? 'bg-indigo-50 border-indigo-200' : 'bg-white hover:bg-slate-50 border-transparent'" class="p-3 border rounded-2xl cursor-pointer transition group"><div class="flex justify-between items-start"><div class="text-sm text-slate-700 truncate flex-1">{{ conv.title }}</div><button @click.stop="deleteConversation(conv.id)" class="text-slate-300 hover:text-rose-400 text-xs ml-2 opacity-0 group-hover:opacity-100 transition">&times;</button></div><div class="text-xs text-slate-400 mt-1 handwritten">{{ conv.date }}</div></div>
        </div>
        <div class="p-3 border-t border-slate-100"><button @click="startNewChat" class="w-full p-2.5 bg-indigo-600 text-white text-sm rounded-xl hover:bg-indigo-700 hover:scale-105 transition">+ 新建对话</button></div>
      </aside>
    </transition>
  </div>
</template>
