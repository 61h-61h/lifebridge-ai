  ...
  <div class="space-y-6">
    ...
    <div v-if="showForm" class="bg-white p-5 rounded-3xl border border-slate-100 shadow-sm space-y-3">
      ...
      <div class="flex items-center gap-3 flex-wrap">
        ...
        <button @click="addTask" :disabled="taskLoading" class="px-6 py-2.5 bg-slate-800 text-white text-sm rounded-xl hover:bg-slate-700 hover:scale-[1.02] transition disabled:opacity-50 btn-save font-body">{{ taskLoading ? '优化中...' : '确认添加' }}</button>
      </div>
    </div>
      ...
      <div v-for="q in quadrants" :key="q.key" class="p-5 rounded-3xl border-2 min-h-[180px]" :class="q.bg + ' ' + q.border">
        ...
        <div class="space-y-2">
          <div v-for="t in getTasks(q.key)" :key="t.id" class="p-3 bg-white rounded-2xl shadow-sm flex items-start gap-2 border border-slate-100" :class="{'opacity-50 line-through':t.done}">
            ...
            <div class="flex-1 min-w-0"><span class="text-slate-700 font-medium text-sm">{{ t.title }}</span><div v-if="t.aiOptimized" class="text-accent-600 mt-1 text-xs">✨ {{ t.aiOptimized }}</div><div v-if="t.optimizing" class="text-slate-400 mt-1 animate-pulse text-xs">优化中...</div></div>
...
<script setup>
import { CheckSquare, Trash2, Sparkles } from 'lucide-vue-next';
