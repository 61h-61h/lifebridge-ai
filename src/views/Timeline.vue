<template>
  <div class="space-y-6">
    <div class="border-b border-slate-100 pb-4 flex justify-between items-center">
      ...
      <button @click="showForm = !showForm" class="px-5 py-2.5 bg-slate-800 text-white text-sm rounded-xl hover:bg-slate-700 hover:scale-[1.02] transition font-body min-h-[44px]" :class="showForm ? 'btn-cancel' : 'btn-add'">{{ showForm ? '取消' : '新增里程碑' }}</button>
    </div>
    <div v-if="showForm" class="bg-white p-5 rounded-3xl border border-slate-100 shadow-sm space-y-3">
      <input v-model="form.title" placeholder="里程碑标题" class="w-full p-3 bg-slate-50 rounded-2xl text-sm outline-none focus:ring-2 focus:ring-slate-200 placeholder:text-slate-300" />
      <input v-model="form.date" type="date" class="w-full p-3 bg-slate-50 rounded-2xl text-sm outline-none text-slate-700" />
      <textarea v-model="form.description" rows="3" placeholder="描述这个里程碑..." class="w-full p-3 bg-slate-50 rounded-2xl text-sm outline-none focus:ring-2 focus:ring-slate-200 resize-none placeholder:text-slate-300"></textarea>
      ...
      <div class="flex gap-2"><button @click="showForm = false" class="flex-1 px-4 py-2.5 bg-slate-100 text-slate-600 text-sm rounded-xl hover:bg-slate-200 transition btn-cancel font-body">取消</button><button @click="addMilestone" class="flex-1 px-6 py-2.5 bg-slate-800 text-white text-sm rounded-xl hover:bg-slate-700 hover:scale-[1.02] transition btn-save font-body">确认添加</button></div>
    </div>
