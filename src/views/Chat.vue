<template>
  <div class="flex-1 flex flex-col min-h-0">
    <div class="border-b border-slate-100 pb-3 flex justify-between items-center shrink-0">
      ...
      <div class="relative">
        <button @click="showHistory = !showHistory" class="px-3 py-2 bg-slate-100 hover:bg-slate-200 rounded-xl text-sm transition flex items-center gap-1.5 text-slate-600 font-body min-h-[44px]">
        ...
        <transition name="dropdown">
          <div v-if="showHistory" class="absolute right-0 top-full mt-2 w-80 bg-white rounded-3xl shadow-lg border border-slate-100 z-30 flex flex-col max-h-[70vh]">
            ...
            <div class="flex-1 overflow-y-auto p-3 space-y-1.5">
              <button @click="startNewChat" class="w-full p-3 text-left bg-primary-50 hover:bg-primary-100 rounded-2xl text-sm text-primary-600 font-medium transition">✏️ 新建对话</button>
              <div v-if="conversations.length === 0" class="text-center text-slate-400 text-sm py-6">暂无历史对话</div>
              <div v-for="conv in conversations" :key="conv.id" @click="loadConversation(conv)" :class="currentConvId === conv.id ? 'bg-primary-50 border-primary-200' : 'bg-white hover:bg-slate-50 border-transparent'" class="p-3 border rounded-2xl cursor-pointer transition group">
                <div class="flex justify-between items-start"><div class="text-sm text-slate-700 truncate flex-1">{{ conv.title }}</div><button @click.stop="deleteConversation(conv.id)" class="text-slate-300 hover:text-rose-400 text-sm ml-2 opacity-0 group-hover:opacity-100 transition shrink-0">&times;</button></div>
    ...
    <div class="py-2 flex gap-1.5 md:gap-2 flex-wrap shrink-0">
      <button v-for="prov in providers" :key="prov.key" @click="selectProvider(prov.key)" :class="selectedProvider === prov.key ? 'bg-primary-600 text-white' : 'bg-white text-slate-500 hover:bg-slate-50 border border-slate-100'" class="px-3 md:px-4 py-2 rounded-xl text-xs md:text-sm transition hover:scale-[1.02] font-body min-h-[40px]">{{ prov.icon }} {{ prov.name }}</button>
    </div>
