<template>
  <div class="p-4 md:p-8 w-full max-w-7xl mx-auto space-y-6">
    <div class="border-b border-slate-100 pb-4 flex justify-between items-center">
      <div><h1 class="text-2xl font-bold text-slate-800">✅ 四象限任务板</h1><p class="text-sm text-slate-400 mt-1">按照重要与紧急程度管理任务</p></div>
    ...
    <div v-if="showForm" class="bg-white p-5 rounded-3xl border border-slate-100 shadow-sm space-y-3">
      ...
      <div class="flex items-center gap-3 flex-wrap">
        <label class="flex items-center gap-1.5 text-sm text-slate-500 cursor-pointer"><input type="checkbox" v-model="form.useAI" class="rounded" /><span>💻 AI 智能优化</span></label>
