<template>
  <div class="p-4 md:p-8 w-full max-w-3xl mx-auto space-y-6">
    <div class="border-b border-slate-100 pb-4"><h1 class="text-2xl font-bold text-slate-800">AI 大脑神经中心</h1><p class="text-sm text-slate-400 mt-1">配置国内大模型密钥，数据纯前端存储在你的个人浏览器中，绝对安全。</p></div>
    ...
    <div class="space-y-4">
      <div v-for="(name, key) in providerNames" :key="key" class="bg-white p-5 rounded-3xl border border-slate-100 shadow-sm grid grid-cols-1 md:grid-cols-3 gap-4 items-center"><span class="font-bold text-slate-700 text-sm">{{ name }} API Key</span><input v-model="keys[key]" type="password" placeholder="请输入 sk-..." class="md:col-span-2 p-3 bg-slate-50 rounded-2xl text-sm font-mono outline-none focus:ring-2 focus:ring-indigo-100 placeholder:text-slate-300" /></div>
      <div class="bg-white p-5 rounded-3xl border border-slate-100 shadow-sm grid grid-cols-1 md:grid-cols-3 gap-4 items-center"><span class="font-bold text-green-600 text-sm">豆包专属 Endpoint ID</span><input v-model="keys['doubao_model']" type="text" placeholder="ep-..." class="md:col-span-2 p-3 bg-slate-50 rounded-2xl text-sm font-mono outline-none focus:ring-2 focus:ring-indigo-100" /></div>
    </div>
