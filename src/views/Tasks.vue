<template>
  <div class="p-4 md:p-8 max-w-6xl mx-auto space-y-6">
    ...
    <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
      <div v-for="q in quadrants" :key="q.key" class="p-4 rounded-3xl border-2 min-h-[150px] md:min-h-[200px]" :class="q.bg + ' ' + q.border">
        <h2 class="font-bold text-sm mb-3" :class="q.text">{{ q.label }}</h2>
        <div class="space-y-2">
          <div v-for="t in getTasks(q.key)" :key="t.id" class="p-2.5 md:p-3 bg-white rounded-2xl text-xs shadow-sm flex items-start gap-2 border border-slate-100" :class="{'opacity-50 line-through':t.done}">
            <input type="checkbox" :checked="t.done" @change="toggleTask(t)" class="mt-0.5" />
            <div class="flex-1"><span class="text-slate-700 font-medium">{{ t.title }}</span><div v-if="t.aiOptimized" class="text-indigo-600 mt-1">✨ {{ t.aiOptimized }}</div><div v-if="t.optimizing" class="text-indigo-400 mt-1 animate-pulse text-[10px]">AI 优化中...</div></div>
            <div class="flex flex-col gap-1"><button @click="optimizeTask(t)" :disabled="t.optimizing" class="text-indigo-400 hover:text-indigo-600 text-[10px]">{{ t.optimizing ? '...' : 'AI' }}</button><button @click="deleteTask(t.id)" class="text-rose-400 hover:text-rose-600 text-[10px] btn-delete">✕</button></div>
          </div>
